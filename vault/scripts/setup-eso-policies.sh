...
# `harbor/broker-credentials` is intentionally NOT created here. If/when
# broker-secret automation is wired up via a vault-secret-store in the
# keycloak namespace, add an `eso-harbor-broker` policy + role.
...
echo ""
echo "=== Removing retired service access ==="
for role in eso-langfuse eso-zot eso-cluster-puller; do
  vault delete "auth/kubernetes/role/${role}" 2>/dev/null && echo "✓ Deleted role: ${role}" || echo "ⓘ Role ${role} not found (already removed)"
done
for policy in eso-langfuse eso-zot eso-cluster-puller; do
  vault policy delete "${policy}" 2>/dev/null && echo "✓ Deleted policy: ${policy}" || echo "ⓘ Policy ${policy} not found (already removed)"
done
